			...
			if (maxExponent > 0) {
				...
				if (maxExponent == 1) {
					...
					if (maxExponent > 1 || maxExponent == maxMaxExponent) {
						g = 0;
						getRealRoot(expressionCoefR, expressionCoefI, maxExponent, -1000);
						while (g < nrRoots) {
							RootR[g] = realRoots[g];
							RootI[g] = 0;
							g++;
						...
						while (g < maxExponent) {
							RootR[g] = RootR[p];
							RootI[g] = RootI[p];
							multiplication(RootR[g], RootI[p], p + g - p, 0);
						...
						while (n < 2000 && countSolutions < maxMaxExponent - 1) {
							g = 0;
							while (g < maxExponent) {
								xValuesR = RootR[g];
								xValuesI = RootI[g];
				...
				while (g < maxExponent) {
					...
					if (isContained("\\", saveEquation) || isContained("x^", saveEquation) ||
						...
						if (rootR > 0 && rootI > 0) {
							...
							sprintf(roots, "%s\\%G+%fi", roots, rootR, rootI);
						}
						else {
							if (rootR > 0 && rootI < 0) {
								sprintf(answers, "%sx%d=%G%Gi\n", answers, rootIndex, rootR, rootI);
