...
- **Must Have:** critical for the core mathematical engine or release safety.
- **Should Have:** important for a complete user workflow but not required for
  minimal calculation capability.
- **Could Have:** desirable improvement or secondary optimization.
- **Won't Have:** intentionally outside the current 2.1.7 scope and tracked in
  future requirements.
...
### 5.1 CLI and Command Environment
...
### 5.2 Parser and Expression Evaluation
...
### 5.3 Arithmetic, Precision and Elementary Functions
...
- **Priority:** Must Have.
- **Depends on:** FR-007, FR-011, FR-014.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
### 5.4 Trigonometry, Hyperbolic Functions and Logarithms
...
### 5.5 Algebra, Polynomial Tools and Solvers
...
- **Priority:** Must Have.
- **Depends on:** FR-007, FR-010, FR-016.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Must Have.
- **Depends on:** FR-007, FR-010, FR-016, FR-022.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Must Have.
- **Depends on:** FR-007, FR-018, FR-019, FR-020, FR-021.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Should Have.
- **Depends on:** FR-007, FR-019, FR-022, FR-024.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
### 5.7 Statistics, DSP and Guided Modules
...
- **Priority:** Should Have.
- **Depends on:** FR-029, FR-030.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
- **Error Handling:** Invalid selections shall be handled.
- **Acceptance Criteria:** Safe smoke path passes; deeper branch coverage is tracked by FRF-004.

### 5.8 Graph, Time and Calendar
...
### 5.9 TXT, File and Command Bridge
...
### 5.10 Persistence, Memory and Errors
...
- **Priority:** Must Have.
- **Depends on:** FR-001, FR-055, FR-056.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Should Have.
- **Depends on:** FR-022, FR-024, FR-030.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Must Have.
- **Depends on:** FR-048, FR-049, FR-053, FR-054.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Should Have.
- **Depends on:** FR-003, FR-004, FR-060.
- **Lifecycle:** Introduced: pre-2.1.7 | Modified: 2.1.7.
...
- **Priority:** Should Have.
- **Depends on:** FR-035, FR-037, FR-038, FR-039, FR-040, FR-041, FR-042, FR-043.
- **Lifecycle:** Introduced: 2.1.7.
...
- **Priority:** Won't Have for 2.1.7.
- **Depends on:** FR-002, FR-007.
...
| Feature / Module | Requirement ID | Test Case Reference | Documentation Section |
| --- | --- | --- | --- |
| CLI prompt and redirected input | FR-001, FR-002 | `run-atc-regression.ps1`; `ATC_AUTOMATED_TEST_CASES.md` CLI cases | README; User Guide; Testing |
| Autocomplete and history | FR-003, FR-004 | Prompt/autocomplete regression cases | Developer Guide; Testing |
| Environment and PC commands | FR-005, FR-006 | Isolated source-level and mocked tests | User Guide Coverage Matrix |
| Parser validation | FR-007, FR-008, FR-009, FR-010 | Parser, syntax and automatic multiplication tests | Architecture; Developer Guide |
| Constants, variables and results | FR-011, FR-012, FR-013, FR-057, FR-058 | Variable/result management tests | User Guide; Testing |
| Arithmetic and precision | FR-014, FR-015, FR-017, FR-056 | Arithmetic and precision regression tests | README; Technical Documentation |
| Complex numbers | FR-016 | Complex polynomial/solver/trigonometry tests | User Guide Full |
| Trigonometry/hyperbolic/logarithms | FR-018, FR-019, FR-020, FR-021 | Function regression cases | User Guide; Cookbook |
| Polynomial engine | FR-022, FR-023, FR-024 | Polynomial, roots and equation tests | Release Notes; User Guide |
| Solvers and function study | FR-025, FR-026, FR-027, FR-028 | Solver, systems and function-study tests | User Guide; Technical Documentation |
| Matrix engine | FR-029, FR-030, FR-031, FR-032, FR-043 | Matrix/list tests | User Guide; Testing |
| Sorting | FR-033 | Sorting and ASCII sorting tests | User Guide Coverage Matrix |
| Statistics and DSP | FR-034, FR-035, FR-036 | Statistics/DSP and guided smoke tests | User Guide; Cookbook |
| Domain guided modules | FR-037, FR-038, FR-039, FR-040, FR-041, FR-042 | Deep interactive safe smoke tests | User Guide Coverage Matrix |
| Graph | FR-044, FR-045 | Graph settings and deterministic navigation tests | Architecture; Testing |
| Time tools | FR-046, FR-047 | Time/calendar regression and isolated guards | Testing |
| TXT and command bridge | FR-048, FR-049, FR-050, FR-051, FR-052, FR-053, FR-054 | TXT flow, command bridge and mocked external-open tests | Testing; User Guide Coverage Matrix |
| Memory management | FR-059, NFR-004 | Memory stress runner | Developer Guide; Release Notes |
| Error handling | FR-060, NFR-007 | Invalid syntax/file/matrix tests | Testing |
| Startup/performance/portability | NFR-001, NFR-002, NFR-003, NFR-008 | Build validation and regression runs | README; Testing |
| Documentation/testability/licensing | NFR-016, NFR-017, NFR-018 | Package validation and documentation checks | README; docs/README.md |
| Future coverage backlog | FRF-001 to FRF-008 | Coverage matrix/manual notes | This SRS; Testing |
...
```text
Regression: 360 passed, 0 failed
