...
void arithmeticMatrixSolver() {
	int op = 0, op1 = 1;
	char* express = getDynamicCharArray(5, "", "express");
	do {
		char* matrix = getDynamicCharArray(5, "", "matrix");
		sprintf(matrix, "");
		int mIndex = 0, ff = 0, lins = 1, cols = 1, lins1 = 1, cols1 = 1, saveCols = -1, saveCols1 = -1, errorCols = 0, errorCols1 = 0, i = 0, j = 0;
		T** vMS, ** uMS, ** vMSI, ** uMSI, ** rMSI, ** rMS;

		vMS = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			vMS[i] = getDynamicDoubleArray();
		}
		uMS = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			uMS[i] = getDynamicDoubleArray();
		}
		vMSI = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			vMSI[i] = getDynamicDoubleArray();
		}
		uMSI = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			uMSI[i] = getDynamicDoubleArray();
		}
		rMSI = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			rMSI[i] = getDynamicDoubleArray();
		}
		rMS = new T* [DIMTWOD];
		for (int i = 0; i < DIMTWOD; ++i) {
			rMS[i] = getDynamicDoubleArray();
		}
		...
		if (op == 1) {
			...
			do {
				...
				sprintf(value, "");
				ff = 0;
				while (matrix[mIndex] != '\\' && matrix[mIndex] != ';' && mIndex < abs((int)strlen(matrix))) {
					value[ff] = matrix[mIndex];
					ff++; mIndex++;
				}
				value[ff] = '\0';
				calcNow<double>(value, 0.0, 0.0);
				vMS[i][j] = resultR;
				vMSI[i][j] = resultI;
				if (matrix[mIndex] == '\\') {
					j++; cols++;
		...
		if (op == 3) {
			...
			do {
				...
				sprintf(value, "");
				ff = 0;
				while (matrix[mIndex] != '\\' && matrix[mIndex] != ';' && mIndex < abs((int)strlen(matrix)) && mIndex < abs((int)strlen(matrix))) {
					value[ff] = matrix[mIndex];
					ff++; mIndex++;
				}
				value[ff] = '\0';
		...
		if (op == 6) {
			...
			if (errorCols == 0 && cols == lins && cols >= 2) {
				char* detChecker = getDynamicCharArray("", "detChecker");
				sprintf(detChecker, "");
				sprintf(detChecker, "det(%s)", matrix);
				atcProg<double>(detChecker);
			...
			_Delete(vMS, DIMTWOD, dime, "vMS,DIMTWOD,dime");

			_Delete(vMSI, DIMTWOD, dime, "vMSI,DIMTWOD,dime");
		...
		while (op1 != 0 && op1 != 1) {
			I_O = true;
			printf("\nContinue? (Yes -> 1 / No -> 0)\n");
			op1 = (int)getValue<double>();
		}

		_Delete(vMS, DIMTWOD, dime, "vMS,DIMTWOD,dime");


		_Delete(uMS, DIMTWOD, dime, "uMS,DIMTWOD,dime");

		_Delete(vMSI, DIMTWOD, dime, "vMSI,DIMTWOD,dime");

		_Delete(uMSI, DIMTWOD, dime, "uMSI,DIMTWOD,dime");

		_Delete(rMSI, DIMTWOD, dime, "rMSI,DIMTWOD,dime");
		_Delete(rMS, DIMTWOD, dime, "rMS,DIMTWOD,dime");
...
template <typename T>
void fmsum(int lins, int cols, T** v, T** u, T** r, T** vI, T** uI, T** rI) {
	int i, j;
...
template <typename T>
void fmsubt(int lins, int cols, T** v, T** u, T** r, T** vI, T** uI, T** rI) {
	int i, j;
	char* report = getDynamicCharArray("", "report");
	sprintf(report, "");
	for (i = 0; i < lins; i++) {
...
void fmdeterminant(int lins, int  cols, T** vMS, T** vMSI) {
	int multiplier = 0;
	do {
		T** newMatrixR, ** newMatrixI;
		newMatrixR = new T* [DIMTWOD];

		for (int i = 0; i < DIMTWOD; ++i) {
			newMatrixR[i] = getDynamicDoubleArray();
		}
		newMatrixI = new T* [DIMTWOD];

		for (int i = 0; i < DIMTWOD; ++i) {
			newMatrixI[i] = getDynamicDoubleArray();
		}

		T* lineMultipliedR = getDynamicDoubleArray(); T* lineMultipliedI = getDynamicDoubleArray(); T* lineResultR = getDynamicDoubleArray(); T* lineResultI = getDynamicDoubleArray();
		int i = 0, j = 0, k = 0, needJacobi = 1, l = 0;
		if (lins > 2) {
			for (i = 0; i < lins; i++) {
				for (j = 0; j < cols; j++) {
					if (precisionValueTo<double>(vMS[i][j]) == 1 && precisionValueTo<double>(vMSI[i][j]) == 0) {
						needJacobi = 0;
					}
				}
			}
			if (needJacobi == 1) {
				...
				do {
					countNotZero = 0;
					i = 0;
					T jacobiMultiplierR = 0, jacobiMultiplierI = 0, jacobiGoalR = 0, jacobiGoalI = 0;
					while (precisionValueTo<double>(vMS[i][j]) == 0 && precisionValueTo<double>(vMSI[i][j]) == 0) {
						i++;
					}
					i++;
					if (precisionValueTo<double>(vMS[i][j]) != 0 || precisionValueTo<double>(vMSI[i][j]) != 0) {
						// Sem ambiguidades, sem conversões lentas em tempo de execução
						multiplication<T>(T(-1), T(0), vMS[i][j], vMSI[i][j]);
						countNotZero++;
					}
					resultR = 1.0 + precisionValueTo<double>(resultR); resultI = precisionValueTo<double>(resultI);
					jacobiGoalR = resultR;
					jacobiGoalI = resultI;
					if (precisionValueTo<double>(vMS[i - 1][j]) == 0 && precisionValueTo<double>(vMSI[i - 1][j]) == 0) {
						l = i + 1;
						while (precisionValueTo<double>(vMS[l][j]) == 0 && precisionValueTo<double>(vMSI[l][j]) == 0 && l < cols) {
							l++;
						}
						if (precisionValueTo<double>(vMS[l][j]) != 0 || precisionValueTo<double>(vMSI[l][j]) != 0) {
							countNotZero++;
							division<double>(precisionValueTo<double>(jacobiGoalR), precisionValueTo<double>(jacobiGoalI), precisionValueTo<double>(vMS[l][j]), precisionValueTo<double>(vMSI[l][j]));
						}
					}
					else {
						countNotZero++;
				...
				if (m != i) {
					for (n = 0; n < cols; n++) {
						if (n != j) {
							...
							multiplication(T(multiplier), T(0), resultR, resultI);
		...
		_Delete(newMatrixR, DIMTWOD, dime, "newMatrixR,DIMTWOD,dime");

		_Delete(newMatrixI, DIMTWOD, dime, "newMatrixI,DIMTWOD,dime");
		_delete(lineMultipliedR, "lineMultipliedR"); lineMultipliedR = nullptr;
		_delete(lineMultipliedI, "lineMultipliedI"); lineMultipliedI = nullptr; _delete(lineResultR, "lineResultR"); lineResultR = nullptr; _delete(lineResultI, "lineResultI"); lineResultI = nullptr;
	} while (lins > 1);
}
template <typename T>
...
void fminverse(int lins, int  cols, T** vMS, T** vMSI, T** inverseR, T** inverseI) {
	...
	for (int i = 0; i < DIMTWOD; ++i) {
		vMSF[i] = getDynamicDoubleArray();
	}
	vMSIF = new T* [DIMTWOD];

	for (int i = 0; i < DIMTWOD; ++i) {
		vMSIF[i] = getDynamicDoubleArray();
	}
	...
	do {
		...
		if (i != pivot) {
			int t = 0;
			T* saveLineR = getDynamicDoubleArray(); T* saveLineI = getDynamicDoubleArray();
			...
			_delete(saveLineR, "saveLineR");
			_delete(saveLineI, "saveLineI");
		}
		i = pivot;
		T pivotR = 0, pivotI = 0;
	...
	_Delete(vMSF, DIMTWOD, dime, "vMSF,DIMTWOD,dime");
	vMSF = nullptr;

	_Delete(vMSIF, DIMTWOD, dime, "vMSIF,DIMTWOD,dime");
	vMSIF = nullptr;
...
void fmpowerm(PrecisionValue** v, PrecisionValue** vI, PrecisionValue** r, PrecisionValue** rI, int power, int lins, int cols) {
	...
	char* report = getDynamicCharArray("", "report");
	sprintf(report, "");
	PrecisionValue prod = 0, prodI = 0;
	PrecisionValue** u, ** uI;

	u = new PrecisionValue* [DIMTWOD];
	for (int i = 0; i < DIMTWOD; ++i) {
		u[i] = getDynamicDoubleArray();
	}

	uI = new PrecisionValue* [DIMTWOD];
	for (int i = 0; i < DIMTWOD; ++i) {
		uI[i] = getDynamicDoubleArray();
	}
	...
	_Delete(u, DIMTWOD, dime, "u,DIMTWOD,dime");

	_Delete(uI, DIMTWOD, dime, "uI,DIMTWOD,dime");
	_delete(report, "report");
	report = nullptr;
}
template <typename T>
int fmrank(int lins, int cols, T** vMS, T** vMSI) {
	...
	do {
		...
		if (i != pivot) {
			...
			_delete(saveLineR, "saveLineR"); _delete(saveLineI, "saveLineI");
		}
		i = pivot + 1;
		T pivotR = 0, pivotI = 0;
		while (i < lins) {
...
void startDetProcessing(char* matrix) {
    startDetProcessing<PrecisionValue>(matrix);
}


template void fmsum<PrecisionValue>(int, int, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**);
template void fmsubt<PrecisionValue>(int, int, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**);
template void fmmulm<PrecisionValue>(int, int, int, int, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**);
template bool fmdivm<PrecisionValue>(int, int, int, int, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**);
template int fmrank<PrecisionValue>(int, int, PrecisionValue**, PrecisionValue**);
template void fminverse<PrecisionValue>(int, int, PrecisionValue**, PrecisionValue**, PrecisionValue**, PrecisionValue**);

void arithmeticMatrixSolver() { arithmeticMatrixSolver<PrecisionValue>(); }
