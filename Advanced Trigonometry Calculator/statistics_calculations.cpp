...
void statisticsCalculations() {
	...
	do {
		...
		if (op == 1) {
			...
			char* population = getDynamicCharArray("", "population");
			gets_s(population, DIM);
			int i = 0, number_of_population = 1;
			for (i = 0; i < abs((int)strlen(population)); i++) {
				if (population[i] == '\\') {
					number_of_population++;
				}
			}
			T* values = getDynamicArray<T>(number_of_population);
			...
			while (p < members) {
				int b = 0;
				while (population[i] != '\\'&&i < abs((int)strlen(population))) {
					toValue[b] = population[i];
					b++; i++;
				}
				toValue[b] = '\0';
				solveMath<T>(toValue);
				values[p] = precisionValueTo<T>(resultR);
				p++;
				sprintf(toValue, "");
				i++;
			}
		...
		if (op == 2) {
			...
			T variance = (T)1.0 / (number_of_population - 1)*summatory;
			printf("\nSample variance: %s\n", convert2Exponential(variance));
			T standard_deviation = sqrt(variance);
		...
		if (op == 3) {
			puts("First number?");
			T first_number = getValue<T>();
			puts("Common difference?");
			T common_difference = getValue<T>();
			puts("The n.th to obtain?");
			int n_th = (int)getValue<T>();
			T* aValues = getDynamicArray<T>(n_th);
			T summation = first_number;
			int b = 0;
			...
			puts(" ");
			_delete(aValues, "aValues"); aValues = nullptr;
		}
		if (op == 4) {
			puts("First number?");
			T first_number = getValue<T>();
			puts("Common ratio?");
			T common_ratio = getValue<T>();
			puts("The n.th to obtain?");
			int n_th = (int)getValue<T>();
			T* aValues = getDynamicArray<T>(n_th);
			T summation = first_number;
		...
		if (op == 6) {
			...
			T confidence_level = getValue<T>() / 100;
			puts("Confidence Interval (%)?");
			T confidence_interval = getValue<T>() / 100;
			puts("Population Proportion (%)?");
			T population_proportion = getValue<T>() / 100;
			puts("Population Size? Info: Enter \"0\" if the size of population is unlimited.");
			T population_size = getValue<T>();
			T z = confidenceLevelToZscore(confidence_level);
			if (z != -77777) {
				T unlimited_population = (pot(z, 2.0, 1)*population_proportion*(1 - population_proportion)) / pot(confidence_interval, 2.0, 1);
				if (population_size == 0) {
					printf("\nYou will need to measure %s or more samples.\n", convert2Exponential(unlimited_population));
				}
				else {
					T finite_population = unlimited_population / (1 + unlimited_population * (1 / population_size));
		...
		if (op == 7) {
			...
			if (z != -77777) {
				if (population_size == 0) {
					T interval = z * sqrt((population_proportion*(1 - population_proportion)) / sample_size);
					printf("The confidence interval is:  %.2f%% - %.2f%%.", (population_proportion - interval) * 100, (population_proportion + interval) * 100);
				}
				else {
					T interval = z * sqrt(((population_proportion*(1 - population_proportion)) / sample_size)*((population_size - sample_size) / (population_size - 1)));
					printf("The confidence interval is:  %.2f%% - %.2f%%.", (population_proportion - interval) * 100, (population_proportion + interval) * 100);
				}
			}
			else {
		...
		if (op == 8) {
			...
			T probability_event_a = getValue<T>();
			puts("Probability of event B?");
			T probability_event_b = getValue<T>();
			if (probability_event_a >= 0 && probability_event_a <= 1 && probability_event_b >= 0 && probability_event_b <= 1) {
				puts("");
				T a_complement = 1 - probability_event_a;
				T b_complement = 1 - probability_event_b;
				...
				T intersection_a_with_b = probability_event_a * probability_event_b;
				T union_a_with_b = probability_event_a + probability_event_b - intersection_a_with_b;
				T exclusive_or_of_a_and_b = union_a_with_b - 2 * intersection_a_with_b;
		...
		if (op == 9) {
			...
			T standard_deviation = getValue<T>();
			puts("Left Bound?");
			T left_bound = getValue<T>();
			puts("Right Bound?");
			T right_bound = getValue<T>();
			T left_probability = 0.5*(1 + gerror((left_bound - mean) / (standard_deviation*sqrt(2.0))));
			T right_probability = 0.5*(1 + gerror((right_bound - mean) / (standard_deviation*sqrt(2.0))));
			puts("");
		...
		if (op == 10) {
			...
			int n = (int)getValue<T>();
			puts("Amount in each Sub-Set?");
			int r = (int)getValue<T>();
			T permutations = fact<T>((T)n) / fact<T>((T)(n - r));
			T combinations = fact<T>((T)n) / (fact<T>((T)r)*fact<T>((T)(n - r)));
		...
		if (op == 11) {
			...
			if (number_of_population % 2 == 1) {
				...
				T median = (values[index_1 - 1] + values[index_2 - 1]) / 2;
				printf("\nMedian: %s", convert2Exponential(median));
			}
			i = 0;
			int j = 0;
			int count = 0;
			int* valuesF = getDynamicArray<int>(number_of_population);
			T* valuesS = getDynamicArray<T>(number_of_population);
			int t = 0;
			...
			T smallest = values[0], largest = values[0];
			i = 0;
			while (i < number_of_population) {
				if (values[i] > largest) {
					largest = values[i];
			...
			_delete(toValue, "toValue"); toValue = nullptr;
			_delete(values, "values"); values = nullptr;
			_delete(valuesF, "valuesF"); valuesF = nullptr;
			_delete(valuesS, "valuesS"); valuesS = nullptr;
		}
		if (op < 1 || op>11) {
...
template<typename T>
T confidenceLevelToZscore(T confidence_level) {
	T confidence[12] = { 0.7,0.75,0.8,0.85,0.92,0.95,0.96,0.98,0.99,0.999,0.9999,0.99999 };
	T z[12] = { 1.04,1.15,1.28,1.44,1.75,1.96,2.05,2.33,2.58,3.29,3.89,4.42 };
	T z_score = 0;
	int i = 0;
...
void statisticsCalculations() { statisticsCalculations<double>(); }
template void statisticsCalculations<double>();
template double confidenceLevelToZscore<double>(double);
