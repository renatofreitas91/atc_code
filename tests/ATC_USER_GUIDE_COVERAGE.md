...
| TXT detector and command bridge | `auto solve txt` live watcher, physical answer-file opening, and `roots to polynomial` inside `solve txt` input files | Direct `predefine txt` -> `solve txt`, including arithmetic, `solver`, `solve equation`, `simplify polynomial`, invalid-line recovery, `enable txt detector`, `atc from cmd`, `to solve`, and `eliminate strings`, is now covered with safe mocks; live watcher behavior and `roots to polynomial` inside TXT still require dedicated fixtures |
