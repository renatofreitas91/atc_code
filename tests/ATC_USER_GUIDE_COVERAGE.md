...
| TXT detector and command bridge | `auto solve txt` live watcher and physical answer-file opening | Direct `predefine txt` -> `solve txt`, `enable txt detector`, `atc from cmd`, `to solve`, and `eliminate strings` are now covered with safe mocks; live watcher behavior still requires a multi-process/file-watcher scenario |
| TXT export-prompt commands | `solve equation` and `simplify polynomial` inside `solve txt` input files | These commands ask their own export prompt and need a prompt-safe TXT-processing path before they should run in the default non-interactive regression suite |
